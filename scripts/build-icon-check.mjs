...
export async function buildIconCheck(root, enabled) {
  ...
  await writeFile(path.join(output, 'index.html'), document('アイコンの比較', '', `<h1>ホーム画面アイコンの比較</h1><p>新しいkondoの透明背景（C）と白背景（D）を比較できます。</p>${cards}<p>A・Bは以前の比較用です。検証用アイコンは普段のkondoとは別のページとして追加されます。</p><small>比較番号 ${revision}</small>`));
