...
export function createTripTransitionController(doc: Document) {
  ...
  const ticket = (id: string) => doc.getElementById(`trip-ticket-${id}`);
  const workspace = (id: string) => doc.getElementById(`trip-workspace-${id}`);
  const list = () => doc.getElementById('trip-list-ready');
  ...
  const run = (direction: Direction, tripId: string, navigate: () => void, animate = true) => {
    ...
    const update = () => op.update ??= (async () => {
      ...
      if (op.decorate) decorate(op, direction === 'open' ? destination : ticket(tripId), direction === 'close');
      focusDestination(op);
    })();
