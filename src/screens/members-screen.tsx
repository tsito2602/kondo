...
export default function MembersScreen() {
  ...
  const load = useCallback(async () => {
    ...
    setError('');
    try {
      if (isDemo) setMembers([{ id: 'demo-self', name: 'あなた', email: '', role: 'owner' }, { id: 'demo-companion', name: '同行者', email: '', role: 'editor' }]);
  ...
  return <View style={styles.screen}>
    <ScrollView contentContainerStyle={[styles.content, { paddingTop: headerHeight + 24 }]}>
      {owner ? <View style={styles.inviteCard}>
      ...
      <View style={styles.listHeading}><Text style={styles.heading}>参加メンバー</Text><Text style={styles.count}>{members.length}人</Text></View>
      {loading ? <ActivityIndicator color={palette.ocean} /> : error ? <View style={styles.inviteCard}><Text style={styles.error}>{error}</Text><Pressable onPress={() => void load()} style={styles.secondary}><Text style={styles.secondaryText}>再読み込み</Text></Pressable></View> : members.map((member) => <View key={member.id} style={styles.member}>
        <View style={styles.memberTop}><View style={styles.avatar}><Text style={styles.initial}>{(member.name || member.email).slice(0, 1).toUpperCase()}</Text></View><View style={styles.memberCopy}><Text style={styles.name}>{member.name || member.email}{member.id === user?.id || member.id === 'demo-self' ? '（あなた）' : ''}</Text>{member.name && member.email ? <Text style={styles.email}>{member.email}</Text> : null}<Text style={styles.role}>{member.role === 'owner' ? '管理者' : member.role === 'editor' ? '編集可' : '閲覧のみ'}</Text></View>
