...
export function MemberPicker({ value, members, onChange }: { value: string; members: TripMember[]; onChange: (value: string) => void }) {
  ...
  return <View accessibilityRole="radiogroup" accessibilityLabel="担当" style={{ gap: 6 }}>
    {options.map((option) => <Pressable key={option.value} accessibilityRole="radio" accessibilityLabel={option.name} accessibilityState={{ checked: value === option.value }} onPress={() => onChange(option.value)} style={{ minHeight: 52, flexDirection: 'row', alignItems: 'center', gap: 12, padding: 10, borderRadius: 12, backgroundColor: value === option.value ? palette.sky : palette.paper }}>
