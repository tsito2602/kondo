...
export function MemberPicker({ value, members, onChange }: { value: string; members: TripMember[]; onChange: (value: string) => void }) {
  ...
  return <View accessibilityRole="radiogroup" accessibilityLabel="担当" style={{ gap: 6 }}>
    {options.map((option) => <Pressable key={option.value} accessibilityRole="radio" accessibilityLabel={option.name} aria-checked={value === option.value} onPress={() => onChange(option.value)} style={{ minHeight: 52, flexDirection: 'row', alignItems: 'center', gap: 12, padding: 10, borderRadius: 12, borderWidth: 1, borderColor: value === option.value ? palette.selectionBorder : 'transparent', backgroundColor: value === option.value ? palette.selection : palette.paper }}>
      ...
      {value === option.value ? <Text style={{ color: palette.ink, fontWeight: '700' }}>✓</Text> : null}
    </Pressable>)}
