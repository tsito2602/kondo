import { useAppTheme, usePalette, useThemedStyles } from '@/theme/theme-provider';
  ...
  return <View style={styles.field}>
    ...
    <View style={styles.group}>
      {endpoints.map((endpoint, index) => <View key={endpoint} style={[styles.row, index > 0 && styles.border]}>
        ...
        {Platform.OS === 'ios' ? <DateTimePicker value={dateFor(endpoint)} mode={showTime ? 'datetime' : 'date'} display="compact" locale="ja-JP" themeVariant={scheme} disabled={disabled} accentColor={palette.ocean} onValueChange={(_, value) => change(endpoint, value, showTime ? 'datetime' : 'date')} style={styles.picker} /> : <View style={styles.values}>
