...
export function FileDrop({ label, hint, accept, multiple = false, disabled = false, onFiles, children }: FileDropProps) {
  ...
  return <div className={`file-drop${dragging ? ' is-dragging' : ''}${unavailable ? ' is-disabled' : ''}`} onDragEnter={(event) => {
    ...
    {children}
    <input ref={input} type="file" accept={accept} multiple={multiple} disabled={unavailable} aria-label={label} style={{ display: 'none' }} onChange={(event) => void submit(Array.from(event.target.files ?? []))} />
    <button type="button" className="file-drop-button" disabled={unavailable} onClick={() => input.current?.click()}>
      ...
      <strong>{processing ? 'ファイルを処理中…' : dragging ? 'ここにドロップ' : label}</strong>
      <span>{hint}</span><span className="file-drop-select">ファイルを選択</span>
    </button>
