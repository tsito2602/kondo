...
export function BookingSheet({ booking, onClose }: { booking?: Booking; onClose: () => void }) {
  ...
  return (
      <FormSheet visible title={viewing ? '予約の詳細' : editingId ? '予約を編集' : '予約を追加'} onClose={onClose} onSave={canEdit ? viewing ? () => setViewing(false) : save : undefined} saveLabel={viewing ? '編集' : '保存'} canSave={viewing || Boolean(draft.title.trim())} dirty={!viewing && (JSON.stringify(draft) !== initialDraft || Boolean(selectedMergeItem))} error={formError}>
