...
export function TripEditor({ trip, onClose, onSaved }: { trip?: Trip; onClose: () => void; onSaved?: (id: string) => void }) {
  ...
  const [initial] = useState(() => JSON.stringify([name, destination, startsOn, endsOn]));
  const save = () => {
    if (!name.trim() || !validDate(startsOn) || !validDate(endsOn) || endsOn < startsOn) return setError('旅行名と正しい旅行期間を入力してください');
    const input = { name: name.trim(), destination: destination.trim(), startsOn, endsOn };
    let id: string;
    if (trip) { updateTrip(trip.id, input); id = trip.id; } else id = createTrip(input);
    onClose(); onSaved?.(id);
  };
  return <FormSheet visible title={trip ? '旅行を編集' : '新しい旅行'} onClose={onClose} onSave={save} canSave={Boolean(name.trim())} dirty={JSON.stringify([name, destination, startsOn, endsOn]) !== initial} error={error}>
    <Text style={styles.label}>旅行名</Text>
    <TextInput accessibilityLabel="旅行名" autoFocus maxLength={100} value={name} onChangeText={setName} placeholder="例：ウィーンの街を歩く" placeholderTextColor={palette.smoke} returnKeyType="next" style={styles.input} />
