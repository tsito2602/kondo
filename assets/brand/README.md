...
`symbol.svg` が文字なしの正本。2枚のチケットの角度を変えて重ね、旅の軌跡とピンを不透明な黒で描く。
...
- 前面の外周は黒 `#000000` の12pxの縁取り。模様も同じ黒で不透明に描く。キャンバスだけを透過にし、チケットと模様には透過の穴を作らない（寸法は1024px原本基準）。
...
現行の調整版Nはピンの外周線を14px、中央円の線を12px（半径10px）へ戻した。Mでは透過の穴の縁取り用の太い線幅26px／24pxが残ってしまい、中央の円がつぶれていた。模様の黒塗りとチケットの配置は維持。
...
- `icon*.png`／`logo*.png`: 1024px、背景は透過、模様は不透明な黒。
- `adaptive-foreground.png`: Android用、セーフエリア内へ縮小。
- `adaptive-monochrome.png`: すべての塗り面を白へ変更したシルエット。
- PWA manifest: `/icons/kondo-icon-v9-192.png`、`-512.png`、`-maskable-512.png`。
- Apple Touch Icon: `/icons/kondo-apple-touch-icon-v9.png`（180px・透過）。
- `*-dark`は通常版と同じデータ。
- 以前のv3/v4/v5/v6/v7/v8画像と実機確認用fixturesは上書きしない。

`EXPO_PUBLIC_ENABLE_DEMO=true npm run build:web`でstaging用の比較ページを生成する。調整版N・別案Oと確認済みIを別IDで追加できる。
