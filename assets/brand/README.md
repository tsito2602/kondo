...
現行の調整版KはIの構造を維持し、前面を白、背面をグレーにして2枚の角度差を20度にしたもの。Iの成功と、変更後のKの実機確認は区別する。Kの実機表示はまだ未確認。
...
- PWA manifest: `/icons/kondo-icon-v6-192.png`、`-512.png`、`-maskable-512.png`。
- Apple Touch Icon: `/icons/kondo-apple-touch-icon-v6.png`（180px・透過）。
- `*-dark`は通常版と同じデータ。
- 以前のv3/v4/v5画像と実機確認用fixturesは上書きしない。

`EXPO_PUBLIC_ENABLE_DEMO=true npm run build:web`でstaging用の比較ページを生成する。調整版Kと確認済みIを別IDで追加できる。
